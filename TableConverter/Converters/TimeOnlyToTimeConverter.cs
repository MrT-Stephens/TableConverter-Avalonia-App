...
public class TimeOnlyToTimeConverter : IValueConverter
{
    ...
    {
        if (value is TimeOnly timeOnly) return timeOnly.ToTimeSpan(); // Convert TimeOnly to TimeSpan
    ...
    {
        if (value is TimeSpan timeSpan) return TimeOnly.FromTimeSpan(timeSpan); // Convert TimeSpan back to TimeOnly
