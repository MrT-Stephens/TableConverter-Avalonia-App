...
public partial class PropertyGridTemplateSelector : ResourceDictionary, IDataTemplate
{
    ...
    private static void ShowSukiHostDialog(ISukiDialogManager manager, TableConverter.Views.Controls.PropertyGrid.ViewModels.ComplexTypeViewModel viewModel)
    {
        manager
            .CreateDialog()
    ...
    protected virtual async void OnMoreInfoClick(object sender, RoutedEventArgs e)
    {
        ...
        if (UseSukiHost)
        {
            ...
            {
                if (control.DataContext is not TableConverter.Views.Controls.PropertyGrid.ViewModels.ComplexTypeViewModel childViewModel || childViewModel.Value is null)
                {
                    return;
                }
