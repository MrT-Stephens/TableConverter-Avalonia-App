...
public static class Drawer
{
    ...
    public static void Show(Control control, object? vm, string? hostId = null, DrawerOptions? options = null)
    {
        var host = OverlayDialogManager.GetHost(hostId, options?.TopLevelHashCode);
    ...
    public static Task<DialogResult> ShowModal(Control control, object? vm, string? hostId = null, DrawerOptions? options = null)
    {
        var host = OverlayDialogManager.GetHost(hostId, options?.TopLevelHashCode);
    ...
    private static void ConfigureCustomDrawer(CustomDrawerControl drawer, DrawerOptions? options)
    {
        ...
        if (options.Position == Position.Left || options.Position == Position.Right)
        {
            if (options.MinWidth is not null) 
                drawer.MinWidth = options.MinWidth.Value;

            if (options.MaxWidth is not null) 
                drawer.MaxWidth = options.MaxWidth.Value;
        }

        if (options.Position is Position.Top or Position.Bottom)
        {
            if (options.MinHeight is not null) 
                drawer.MinHeight = options.MinHeight.Value;

            if (options.MaxHeight is not null) 
                drawer.MaxHeight = options.MaxHeight.Value;
        }
