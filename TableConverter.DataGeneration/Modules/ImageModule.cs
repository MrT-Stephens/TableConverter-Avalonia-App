    ...
    : ModuleBase(faker, locale, randomizer)
{
    ...
    public virtual string UrlLoremFlickr(int width = 640, int height = 480, string category = "")
    {
        if (width is < 1 or > 3999)
            FakerArgumentOutOfRangeException.CreateException<object>(width, "Width must be between 1 and 3999");

        if (height is < 1 or > 3999)
            FakerArgumentOutOfRangeException.CreateException<object>(height, "Height must be between 1 and 3999");
    ...
    public virtual string UrlPicsumPhotos(int width = 640, int height = 480, bool grayscale = false, int blur = 0)
    {
        ...
        if (blur is < 0 or > 10)
            FakerArgumentOutOfRangeException.CreateException<object>(blur, "Blur must be between 0 and 10");
        ...
        DataUriTypeEnum dataUriType = DataUriTypeEnum.SvgUri)
    {
        ...
        if (string.IsNullOrEmpty(color))
            ...
            FakerArgumentException.CreateException<object>(color, "Invalid color format. Must be a hex color code");
        ...
        return dataUriType switch
        {
            ...
            _ => FakerArgumentException.CreateException<string>(dataUriType, "Invalid image type specified")
        };
    }
}