    ...
    public class DataGenerationMoviesHandler : DataGenerationTypeHandlerAbstract<DataGenerationMoviesOptions>
    {
        protected override string[] GenerateDataOverride(int rows, ushort blanks_percentage)
        {
            ...
            using (var reader = DbConnection.ExecuteCommand(
                @$"SELECT M.NAME FROM MOVIES_TABLE M 
                   INNER JOIN MOVIES_TO_GENRES_MAP_TABLE MG ON M.ID = MG.MOVIE_ID 
                   INNER JOIN MOVIES_GENRES_TABLE G ON MG.GENRE = G.GENRE 
                   WHERE G.GENRE = '{Options!.MovieGenre}' AND M.ID IN (
                      SELECT ID FROM MOVIES_TABLE 
                      WHERE ID IN (
                         SELECT MG.MOVIE_ID FROM MOVIES_TO_GENRES_MAP_TABLE MG 
                         JOIN MOVIES_GENRES_TABLE G ON MG.GENRE = G.GENRE WHERE G.GENRE = '{Options!.MovieGenre}'
