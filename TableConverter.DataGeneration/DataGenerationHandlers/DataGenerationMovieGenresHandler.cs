    ...
    public class DataGenerationMovieGenresHandler : DataGenerationTypeHandlerAbstract<DataGenerationBaseOptions>
    {
        protected override string[] GenerateDataOverride(int rows, ushort blanks_percentage)
        {
            ...
            using (var reader = DbConnection.ExecuteCommand(
                @$"SELECT M.NAME, GROUP_CONCAT(G.GENRE, ', ') FROM MOVIES_TABLE M 
                   JOIN MOVIES_TO_GENRES_MAP_TABLE MG ON M.ID = MG.MOVIE_ID JOIN MOVIES_GENRES_TABLE G ON MG.GENRE = G.GENRE 
                   WHERE M.ID IN (
                      SELECT ID FROM MOVIES_TABLE ORDER BY RANDOM() LIMIT {rows}
                   ) GROUP BY M.ID;"
            ))
            {
                if (!reader.HasRows)
