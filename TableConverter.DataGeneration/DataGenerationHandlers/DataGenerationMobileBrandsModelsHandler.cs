...
namespace TableConverter.DataGeneration.DataGenerationHandlers
{
    ...
    {
        protected override string[] GenerateDataOverride(int rows, DataGenerationMobileBrandsModelsOptions? options, ushort blanks_percentage)
        {
            string[] data = new string[rows];

            using (var reader = DbConnection.ExecuteCommand(
                $"SELECT MM.NAME FROM MOBILE_BRANDS_MODELS_TABLE MM INNER JOIN MOBILE_BRANDS_TABLE MB ON MB.ID = MM.BRAND_ID WHERE MB.NAME = '{options!.MobileBrand}' AND MM.ID IN (SELECT ID FROM MOBILE_BRANDS_MODELS_TABLE WHERE BRAND_ID = MB.ID ORDER BY RANDOM() LIMIT {rows});"
            ))
            {
                if (!reader.HasRows)
                {
