...
using TableConverter.Utilities.Logging.Format;
...
public class StringLogEntryFormatterTests
{
	...
	[InlineData("t1", LogLevel.Warning, int.MaxValue, "Error", "Invalid operation")]
	[InlineData("t2", LogLevel.Debug, int.MinValue, "Error", "Invalid operation")]
	...
	public void GetFormattedLength()
	{
		...
		for (int i = int.MaxValue; i > 0; i = i / 10)
		{
			Assert.Equal(GetFormattedLengthByFormula(i), i.GetFormattedLength());
		}

		for (int i = int.MinValue; i < 0; i = i / 10)
			Assert.Equal(GetFormattedLengthByFormula(i), i.GetFormattedLength());
