    ...
    public class ConverterHandlerAsciiOutput : ConverterHandlerOutputAbstract<ConverterHandlerAsciiOutputOptions>
    {
        public override Result<string> Convert(string[] headers, string[][] rows)
        {
            ...
            foreach (var row in rows)
            {
                ...
                {
                    maxColumnWidths[i] = Math.Max(maxColumnWidths[i], row[i].ToString().LongCount() + 2);
                }
            }

            var comment = Options!.SelectedCommentType == Options!.CommentTypes.First().Key ? "" : $"{Options!.CommentTypes[Options!.SelectedCommentType]}    ";
        ...
        private static string DrawSeparator(long[] columnWidths, char leftChar, char rightChar, char intersectionChar, char fillChar)
        {
        ...
        private static string DrawDataRow(string[] row, long[] columnWidths, TextAlignment textAlignment, char leftChar, char rightChar, char intersectionChar)
        {
