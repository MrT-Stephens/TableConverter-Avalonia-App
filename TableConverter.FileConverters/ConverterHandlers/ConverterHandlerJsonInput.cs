    ...
    public override Result<TableData> ReadText(string text)
    {
        ...
        try
        {
            ...
            bool ValidateColumnArrays(List<Dictionary<string, string[]>> jsonObjects)
            {
                ...
                var columnLengths = jsonObjects.SelectMany(obj => obj.Values).Select(arr => arr.Length).Distinct()
                    .ToList();
            ...
            switch (Options!.SelectedJsonFormatType)
            {
                case "Array of Objects":
                {
                    ...
                    {
                        foreach (var key in jsonObject.Keys.Where(key => !headers.Contains(key))) headers.Add(key);

                        rows.Add(headers.ConvertAll(header =>
                                jsonObject.TryGetValue(header, out var value) ? value.ToString() ?? "" : string.Empty)
                            .ToArray());
                    }
                ...
                case "2D Arrays":
                {
                    ...
                    for (var i = 1; i < jsonArrays.Count; i++)
                ...
                case "Column Arrays":
                {
                    ...
                    for (var i = 0; i < jsonObjects.Count; i++)
                    {
                        ...
                        {
                            if (i == 0) rows.Add(new string[jsonObjects.Count]);
                ...
                case "Keyed Arrays":
                {
                    ...
                    for (var i = 1; i < jsonObjects.Count; i++) rows.Add(jsonObjects[i].Values.First().ToArray());
